...
class Users extends Database {
	...
	protected function register($username, $email, $discord, $hashedPassword, $invCode) {
		...
		$this->prepare('INSERT INTO `users` (`username`, `password`, `email`, `discord`, `invitedBy`) VALUES (?, ?, ?, ?, ?)');

		// If user registered
		if ($this->statement->execute([$username, $email, $discord, $hashedPassword, $inviter])) {
	...
	protected function subscription($subCode, $username) {
		...
		if ($sub <= 0) {
			...
			$date = new DateTime(); // Get current date
			$date->add(new DateInterval('P30D')); // Adds 32 days
